...
static  void  ConfigLEDGPIO(void)
{
  ...
  GPIO_WriteBit(GPIOA, GPIO_Pin_8, Bit_RESET);            //将LED1默认状态设置为点亮

  GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_2;           //设置引脚
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;     //设置I/O输出速度
  GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP;     //设置模式
  GPIO_Init(GPIOD, &GPIO_InitStructure);                //根据参数初始化LED2的GPIO

  GPIO_WriteBit(GPIOD, GPIO_Pin_2, Bit_SET);          //将LED2默认状态设置为熄灭
}
...
*********************************************************************************************************/
void LEDFlicker(uint16 cnt)
{
  static uint16 s_iCnt;  //定义静态变量s_iCnt作为计数器
